...

{
  config = lib.mkMerge [{
    nixpkgs.overlays = [
      (final: prev: {
        oqs-openssh-unwrapped = prev.stdenv.mkDerivation rec {
          ...
          meta = with prev.lib; {
            description =
              "OpenSSH fork with post-quantum crypto (OQS) - unwrapped";
        ...
        sshfs = prev.stdenv.mkDerivation rec {
          pname = "oqs-sshfs";
          version = "1.0";
          ...
          nativeBuildInputs = [ final.makeWrapper ];
          buildInputs = [ prev.sshfs final.oqs-openssh ];
          ...
          meta = with prev.lib; {
            description =
              "SSHFS wrapper that uses OQS OpenSSH via -o ssh_command";
            homepage = "https://github.com/open-quantum-safe/openssh";
            license = licenses.bsd3;
          };
        };
