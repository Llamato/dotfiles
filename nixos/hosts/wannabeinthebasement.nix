...
let
  ...
  makeUser =
    ...
    let
      ...
      getKeyFiles =
        with builtins;
        dir: map (file: readFile file) (lib.fileset.fileFilter (file: endsWith ".pub" file.name) dir);

      systemKeyFiles =
        username:
        let
          keyDir = "/mnt/raid/home/tina/dotfiles/keys";
        in
        lib.fileset.fileFilter (file: startsWith username file.name) keyDir;
